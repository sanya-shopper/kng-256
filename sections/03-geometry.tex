...
Python, about a minute of computation, self-testing against brute-force
enumeration on small domains before reporting anything). Narrow every
...
\begin{table}[ht]
  ...
  (\texttt{tools/centralizer\_census.py}). $|C_{\mathrm{Sym}}|$ is the
...
\begin{project}{Single operations under the $2$-adic lens}{elementary
  ...
  (\texttt{tools/centralizer\_census.py} already runs the $w=8$
  warm-up---the single-orbit checks, the rotation ablation, and the
  centralizer census of \cref{tab:census}---and is written to be
  extended.)
\end{project}
